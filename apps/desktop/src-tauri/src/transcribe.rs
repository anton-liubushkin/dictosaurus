...
//! - GigaAM v3 (ONNX) via `sherpa-onnx`
//!
...
fn preload(model_id: &str) -> Result<bool, String> {
    let (Some(def), Some(paths)) = (
        models::def_by_id(model_id),
        models::resolve_model_paths(model_id),
    ) else {
        return Ok(false);
    };
    with_loaded(def.engine, &paths, |_| Ok(()))?;
    Ok(true)
}
    ...
    if !cached {
        ...
        *guard = None;
        let loaded = match engine {
            ...
            Engine::Gigaam => Loaded::Gigaam(load_gigaam(key, &paths[1])?),
        };
...
/// whisper language code or "auto" (ignored by the Russian-only GigaAM).
pub fn transcribe(model_id: &str, language: &str, pcm: &[f32]) -> Result<String, String> {
    ...
    let paths = models::resolve_model_paths(model_id)
        .ok_or_else(|| "model files are missing".to_string())?;

    with_loaded(def.engine, &paths, |loaded| match loaded {
        Loaded::Whisper(ctx) => run_whisper(ctx, language, pcm),
        Loaded::Gigaam(recognizer) => run_gigaam(recognizer, pcm),
    })
}
...
// --- GigaAM v3 (sherpa-onnx, NeMo CTC) ---

fn load_gigaam(model: &Path, tokens: &Path) -> Result<OfflineRecognizer, String> {
    let mut config = OfflineRecognizerConfig::default();
    // GigaAM expects 64 mel bins (sherpa-onnx defaults to 80).
    ...
    config.model_config.num_threads = std::thread::available_parallelism()
        ...
        .unwrap_or(4)
        .min(4);
    OfflineRecognizer::create(&config).ok_or_else(|| "failed to load GigaAM model".to_string())
}

fn run_gigaam(recognizer: &OfflineRecognizer, pcm: &[f32]) -> Result<String, String> {
